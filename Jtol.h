...
namespace Jtol{
    ...
    const char* NetGet(Net sock){
        ...
        if(recv(sock,&(NetBuf[sock][0]),NetBuf[sock].size(),0)!=SOCKET_ERROR){
        ...
        Timeout.tv_sec = TimeoutSec;
        Timeout.tv_usec = 0;
        int r=select(0,NULL,&Write,&Err,&Timeout);
        if(r==0){
            //printf("Connect Timeout!\n");
            //MessageBoxA(0, "Connect Timeout!", "DLL Message", MB_OK | MB_ICONINFORMATION);
            return 0;
            ...
            if(!FD_ISSET(sock, &Write)&&FD_ISSET(sock, &Err)){
                //printf("Select error!\n");
                return 0;
                }
            }
    ...
    Thread ThreadCreat(T will_run, Args... args){
    ...
    Color GetColFast(int x,int y){
        if(Background_Update_Screen_Off==1)
            ...
            Background_Update_Screen_Thread=ThreadCreat(Background_Update_Screen_Func);
